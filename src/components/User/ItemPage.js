...
export default function ItemPage() {
  ...
  const { item , loading} = useSelector((state) => state.admin);

  const [selectedImage, setSelectedImage] = useState(null);

  const itemName = useParams().id;
  // replace all the dashes with spaces and make the first letter of each word uppercase
  const itemNewName = itemName.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase());
  ...
  useEffect(() => {
    getAdminItemByTitle(dispatch, itemNewName);
    window.scrollTo(0, 0);
    document.title = itemNewName;
  }, [dispatch, itemNewName]);

  return (
    <>
      {loading ? (
        <Center w="100%" h="70vh" align="center" justify="center">
